...
class _ProjectOverviewScreenState extends State<ProjectOverviewScreen>
  ...
  Widget _buildGlobalKpi() {
    ...
    final hasActive = (((_globalStats['active_teams'] as num?) ?? 0) > 0);
    final cum = (_globalStats['cumulative'] as Map<String, dynamic>?) ?? {};
    final activeTeams = _globalStats['all_teams'] ?? _globalStats['active_teams'] ?? 0;
    final totalTickets = hasActive ? (_globalStats['total_tickets'] ?? 0) : (cum['total_tickets'] ?? 0);
    final doneTickets = hasActive ? (_globalStats['done_tickets'] ?? 0) : (cum['done_tickets'] ?? 0);
    final globalProgress = hasActive ? (_globalStats['global_progress'] ?? 0) : (cum['global_progress'] ?? 0);
    final activeSprints = _sprintStats['active_sprints'] ?? 0;
    final blocked = hasActive ? (_globalStats['blocked_tickets'] ?? 0) : (cum['blocked_tickets'] ?? 0);
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  Container(
                    ...
                    child: Center(child: Text(level.icon, style: const TextStyle(fontSize: 20))),
                  ),
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    if (proj.description.isNotEmpty) ...[
                      const SizedBox(height: 2),
                      Text(proj.description, maxLines: 2, overflow: TextOverflow.ellipsis,
                  ...
                  Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                    ...
                    Text('${proj.doneTickets}/${proj.totalTickets}',
                        style: const TextStyle(color: Color(0xFF8b949e), fontSize: 10)),
                  ]),
                ]),
                if (proj.goalTitle.isNotEmpty) ...[
                  const SizedBox(height: 6),
                  Text('\u{1F3AF} ${proj.goalTitle}',
                      style: const TextStyle(color: Color(0xFF4AC99B), fontSize: 12, fontWeight: FontWeight.w600)),
                ],
                const SizedBox(height: 8),
                // 태그 배지 행
                Wrap(spacing: 4, runSpacing: 4, children: [
                  ...
                  if (skillCnt > 0) _tagBadge('$skillCnt skills', const Color(0xFFf0883e)),
                  if (mcpList.isNotEmpty) _tagBadge('MCP:${mcpList.length}', const Color(0xFF4AC99B)),
                  if (proj.archivedTeams > 0) _tagBadge('${proj.archivedTeams} archived', const Color(0xFF484f58)),
                ]),
                const SizedBox(height: 8),
                // 프로그레스 바
                ClipRRect(
                ...
                Row(children: [
                  ...
                  _legendDot('Review ${proj.reviewTickets}', const Color(0xFFf0883e)),
                  if (proj.blockedTickets > 0) _legendDot('Block ${proj.blockedTickets}', const Color(0xFFf85149)),
                  _legendDot('Remain ${proj.remaining}', const Color(0xFF8b949e)),
                ]),
                const SizedBox(height: 6),
                // KPI 행
                Container(
                  ...
                  child: Row(children: [
                    _popupKpi('진행률', '${proj.progress.toInt()}%', const Color(0xFF1B96FF)),
                    Container(width: 1, height: 20, color: const Color(0xFF30363d)),
                    _popupKpi('잔여', '${proj.remaining}', const Color(0xFFf0883e)),
                    Container(width: 1, height: 20, color: const Color(0xFF30363d)),
                    _popupKpi('팀', '${proj.activeTeams}', const Color(0xFF7c3aed)),
                    Container(width: 1, height: 20, color: const Color(0xFF30363d)),
                    _popupKpi('완료', '${proj.doneTickets}', const Color(0xFF4AC99B)),
                    Container(width: 1, height: 20, color: const Color(0xFF30363d)),
                    _popupKpi('Blocked', '${proj.blockedTickets}',
                        proj.blockedTickets > 0 ? const Color(0xFFf85149) : const Color(0xFF8b949e)),
                  ]),
                ),
              ]),
            ),
  ...
  Widget _popupKpi(String label, String value, Color color) {
    return Expanded(
      child: Column(children: [
        Text(value, style: TextStyle(color: color, fontSize: 13, fontWeight: FontWeight.w800, fontFamily: 'monospace')),
        Text(label, style: const TextStyle(color: Color(0xFF8b949e), fontSize: 9)),
      ]),
    );
  }
...
class _TeamHistoryPageState extends State<_TeamHistoryPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (mounted) setState(() {
        _items = ((res['history'] as List?) ?? []).cast<Map<String, dynamic>>();
        _loading = false;
      });
    } catch (e) {
      if (mounted) setState(() { _error = '히스토리 로드 실패: $e'; _loading = false; });
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _loading
        ? const Center(child: CircularProgressIndicator(strokeWidth: 2, color: Color(0xFF1B96FF)))
        : _error != null
        ? ErrorView(message: '히스토리를 불러올 수 없습니다', detail: _error, onRetry: _load)
        : _items.isEmpty
        ? const EmptyView(icon: Icons.history, title: '히스토리 없음', subtitle: '아직 기록된 활동이 없습니다')
        : ListView.builder(
            padding: const EdgeInsets.all(8),
...
class _AgentListBodyState extends State<_AgentListBody> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (mounted) setState(() { _error = '에이전트 로드 실패: $e'; _loading = false; });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_error != null) return ErrorView(message: '에이전트를 불러올 수 없습니다', detail: _error, onRetry: _load);
    if (_agents.isEmpty) return const EmptyView(icon: Icons.smart_toy_outlined, title: '에이전트 없음');
