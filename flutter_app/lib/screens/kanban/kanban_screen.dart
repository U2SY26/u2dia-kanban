...
import 'agent_office_screen.dart';
...
class _KanbanScreenState extends State<KanbanScreen> {
  ...
  @override
  void dispose() {
    _timer?.cancel(); _hScroll.dispose();
        ...
        actions: [
          TextButton(
            onPressed: () => Navigator.push(context, MaterialPageRoute(
              builder: (_) => AgentOfficeScreen(teamId: widget.teamId, teamName: widget.teamName),
            )),
            ...
            child: Container(
              ...
              child: const Text('Office', style: TextStyle(fontSize: 11, color: Color(0xFF58a6ff), fontWeight: FontWeight.w600)),
            ),
          ),
  ...
  Future<void> _changeStatus(String ticketId, String status) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text('$ticketId → $status', style: const TextStyle(color: Color(0xFFe6edf3), fontSize: 14)),
        content: Text('상태를 $status로 변경하시겠습니까?', style: const TextStyle(color: Color(0xFF8b949e), fontSize: 13)),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('취소', style: TextStyle(color: Color(0xFF8b949e)))),
          ElevatedButton(
    ...
    if (mounted) {
      if (res['ok'] != true) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(res['message']?.toString() ?? res['error']?.toString() ?? '변경 실패'),
