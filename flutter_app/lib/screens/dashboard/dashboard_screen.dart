...
import '../operations/competition_detail_screen.dart';

class DashboardScreen extends StatefulWidget {
  final Function(String, String)? onTeamTap;
  final Function(int)? onNavigateToTab;
  final Function(int)? onNavigateToOperations;
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _loadKpi() async {
    ...
    try {
      ...
      final r7 = await api.getCompetitions();
      if (mounted) {
        setState(() {
      ...
      if (mounted) {
        setState(() => _bannerError = 'KPI 로드 실패 — 서버 연결 확인 필요');
      }
    }
  }
  ...
  Future<void> _loadCharts() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() => _bannerError = '차트 데이터 로드 실패');
      }
    }
  }
              ...
              child: ListView(padding: const EdgeInsets.all(12), children: [
                if (_bannerError != null)
                  InlineErrorBanner(
                    ...
                    onRetry: () { setState(() => _bannerError = null); _loadAll(); },
  ...
  Widget _buildServerHealth() {
    ...
    switch (pHealth) {
      ...
      case 'idle': pColor = const Color(0xFF58a6ff); break;
  ...
  Widget _buildKpiGrid() {
    ...
    // 활성 팀 없으면 누적 통계 사용
    final cum = (s['cumulative'] as Map<String, dynamic>?) ?? {};
    final hasActive = ((s['active_teams'] as num?) ?? 0) > 0;
    final effTickets = hasActive ? (s['total_tickets'] ?? 0) : (cum['total_tickets'] ?? 0);
    final effDone = hasActive ? (s['done_tickets'] ?? 0) : (cum['done_tickets'] ?? 0);
    final effBlocked = hasActive ? (s['blocked_tickets'] ?? 0) : (cum['blocked_tickets'] ?? 0);
    final effAgents = hasActive ? (s['total_agents'] ?? 0) : (cum['total_agents'] ?? 0);
    final effProgress = hasActive ? (s['global_progress'] ?? 0) : (cum['global_progress'] ?? 0);

    // onTap 라우팅: (mainTab, operationsSubTab?) — operationsSubTab이 null이면 메인 탭만 이동
    final cards = [
      _KpiData('Teams', '${s['all_teams'] ?? 0}', Icons.groups, const Color(0xFF58a6ff),
          sub: '${s['active_teams'] ?? 0} active / ${s['archived_teams'] ?? 0} arch',
          onTap: () => widget.onNavigateToTab?.call(1)),
      _KpiData('Agents', '$effAgents', Icons.smart_toy, const Color(0xFF39d2c0),
          sub: hasActive ? '${s['working_agents'] ?? 0} working' : 'cumulative',
          onTap: () => widget.onNavigateToTab?.call(1)),
      _KpiData('Tickets', '$effTickets', Icons.confirmation_number, const Color(0xFFbc8cff),
          sub: '$effDone done',
          onTap: () => widget.onNavigateToTab?.call(2)),
      _KpiData('Progress', '$effProgress%', Icons.pie_chart, const Color(0xFF3fb950),
          sub: hasActive ? null : 'cumulative',
          onTap: () => widget.onNavigateToTab?.call(1)),
      _KpiData('Blocked', '$effBlocked', Icons.block,
          (effBlocked as num) > 0 ? const Color(0xFFf85149) : const Color(0xFF8b949e),
          onTap: () => widget.onNavigateToOperations?.call(3)), // Hooks 서브탭
      _KpiData('Tokens', _fmtNum(((ut['input_tokens'] ?? 0) + (ut['output_tokens'] ?? 0)) as num),
          Icons.token, const Color(0xFF39d2c0),
          onTap: () => widget.onNavigateToOperations?.call(2)), // 자원 서브탭
      _KpiData('Total Cost', _fmtKrw((ut['total_cost'] ?? 0) as num), Icons.payments, const Color(0xFFe3b341),
          sub: '\$${((ut['total_cost'] ?? 0) as num).toStringAsFixed(2)}',
          onTap: () => widget.onNavigateToOperations?.call(2)),
      _KpiData('Today', _fmtKrw((utd['total_cost'] ?? 0) as num), Icons.today, const Color(0xFF7ee787),
          sub: '\$${((utd['total_cost'] ?? 0) as num).toStringAsFixed(2)}',
          onTap: () => widget.onNavigateToOperations?.call(2)),
      _KpiData('System', '${m['cpu_percent'] ?? '-'}%', Icons.monitor_heart, const Color(0xFF8b949e),
          sub: '${m['memory_percent'] ?? '-'}% RAM',
          onTap: () => widget.onNavigateToOperations?.call(2)),
    ];
    ...
    return GestureDetector(
          ...
          builder: (_) => Padding(padding: const EdgeInsets.all(20),
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              FittedBox(
                ...
                child: Text(d.value, style: TextStyle(color: d.color, fontSize: 28, fontWeight: FontWeight.w800, fontFamily: 'monospace'), maxLines: 1),
              ),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
        ...
        FittedBox(
          ...
          child: Text(d.value, style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: d.color, fontFamily: 'monospace'), maxLines: 1),
        ),
        if (d.sub != null) Text(d.sub!, style: const TextStyle(fontSize: 9, color: Color(0xFF8b949e)), maxLines: 1, overflow: TextOverflow.ellipsis),
      ]),
    ));
  }
  ...
  Widget _buildPipelineInsight() {
    // 활성 팀 없으면 누적 통계 사용
    final cumP = (_pipeline['cumulative'] as Map<String, dynamic>?) ?? {};
    final activeSc = (_pipeline['status_counts'] as Map<String, dynamic>?) ?? {};
    final hasActiveTickets = activeSc.isNotEmpty;
    final sc = hasActiveTickets ? activeSc : ((cumP['status_counts'] as Map<String, dynamic>?) ?? {});
    ...
    final total = hasActiveTickets ? ((_pipeline['total_tickets'] as num?)?.toInt() ?? 0) : ((cumP['total_tickets'] as num?)?.toInt() ?? 0);
    final rate = (_pipeline['completion_rate'] as num?)?.toDouble() ?? 0;
    // onTap: Operations > Hooks 서브탭
    final insightCard = _buildPipelineInsightInner(sc, health, issues, last24, total, rate);
    return GestureDetector(
      onTap: () => widget.onNavigateToOperations?.call(3),
      child: insightCard,
    );
  }

  Widget _buildPipelineInsightInner(
      Map<String, dynamic> sc, String health, List<String> issues,
      Map<String, dynamic> last24, int total, double rate) {

    Color hc;
    String hl;
    switch (health) {
      case 'healthy': hc = const Color(0xFF3fb950); hl = 'HEALTHY'; break;
      case 'idle': hc = const Color(0xFF58a6ff); hl = 'IDLE'; break;
  ...
  Widget _buildProjectProgress() {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      ...active.take(6).map((p) {
        ...
        return GestureDetector(
          onTap: () => widget.onNavigateToTab?.call(1), // 프로젝트 탭
          child: Container(
  ...
  Widget _buildCompetitions() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      GestureDetector(
        onTap: () => widget.onNavigateToOperations?.call(0),
        child: Row(children: [
          const Icon(Icons.emoji_events, size: 16, color: Color(0xFFd29922)),
          const SizedBox(width: 6),
          Text('Competitions (${_competitions.length})', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Color(0xFFe6edf3))),
          const SizedBox(width: 6),
          const Icon(Icons.chevron_right, size: 14, color: Color(0xFF8b949e)),
        ]),
      ),
      const SizedBox(height: 8),
      ..._competitions.map((c) {
        ...
        final avg = (c['avg_score'] as num?)?.toDouble();
        final avgStr = avg != null ? avg.toStringAsFixed(1) : '-';
        final totalEvents = (c['total_events'] as num?)?.toInt() ?? 0;
        final activeTeams = (c['active_team_count'] as num?)?.toInt() ??
            ((c['teams'] as List?) ?? []).where((t) => t['archived'] != true).length;
        ...
        if (deadline.isNotEmpty) {
          try {
            ...
            dday = diff > 0 ? 'D-$diff' : diff == 0 ? 'D-DAY' : 'D+${-diff}';
          } catch (_) {}
        }

        final scoreColor = avg != null && avg >= 4
        ...
        final progressColor = progress >= 100
            ...
            : progress >= 50
                ...
                : const Color(0xFFd29922);
        final ddayColor = dday == 'D-DAY' || dday.startsWith('D+')
            ? const Color(0xFFf85149)
            : dday.isNotEmpty
                ? const Color(0xFFd29922)
        ...
        return GestureDetector(
          ...
          decoration: BoxDecoration(
            color: const Color(0xFF161b22),
            border: Border(left: BorderSide(color: progressColor, width: 3)),
            borderRadius: BorderRadius.circular(8),
          ),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            // 제목 + 뱃지
            Row(children: [
              Expanded(
                child: Text(title, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Color(0xFFe6edf3)),
              ...
              if (dday.isNotEmpty) ...[
                Container(
                  ...
                  decoration: BoxDecoration(color: ddayColor.withOpacity(0.15), borderRadius: BorderRadius.circular(6)),
                  child: Text(dday, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: ddayColor, fontFamily: 'monospace')),
                ),
              ],
              if (totalCost > 0) ...[
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                  margin: const EdgeInsets.only(right: 4),
                  decoration: BoxDecoration(color: const Color(0xFFd29922).withOpacity(0.15), borderRadius: BorderRadius.circular(6)),
                  child: Text('\$${totalCost > 1000 ? "${(totalCost/1000).toStringAsFixed(1)}K" : totalCost.toStringAsFixed(0)}',
                      style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: Color(0xFFd29922), fontFamily: 'monospace')),
                ),
              ],
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                decoration: BoxDecoration(color: scoreColor.withOpacity(0.15), borderRadius: BorderRadius.circular(6)),
                child: Text(avgStr, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: scoreColor, fontFamily: 'monospace')),
              ),
            ]),
            // 프로그레스 바
            if (totalTickets > 0) ...[
              ...
              Row(children: [
                ...
                const SizedBox(width: 8),
                Text('${progress.toStringAsFixed(0)}%', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: progressColor, fontFamily: 'monospace')),
              ]),
            ],
            const SizedBox(height: 6),
            // 통계 행
            Row(children: [
              _compStat('Tickets', '$doneTickets/$totalTickets', const Color(0xFF3fb950)),
              _compStat('Events', '$totalEvents', const Color(0xFF8b949e)),
              _compStat('Teams', '$activeTeams', const Color(0xFF8B5CF6)),
              if (lambdaCost > 0)
                _compStat('GPU', '\$${lambdaCost.toStringAsFixed(0)}', const Color(0xFF58a6ff))
              else
                _compStat('Score', avgStr, scoreColor),
            ]),
          ]),
          ),
        );
  ...
  Widget _compStat(String label, String value, Color color) {
    return Expanded(
      child: Row(children: [
  ...
  Widget _teamTile(Map<String, dynamic> t) {
    ...
    final done = (t['done_tickets'] ?? stats['done_tickets'] ?? 0) as int;
    final total = (t['total_tickets'] ?? stats['total_tickets'] ?? 0) as int;
    final pct = total > 0 ? done / total : 0.0;
    return GestureDetector(
  ...
  Widget _actTile(Map<String, dynamic> a) {
    ...
    final teamId = (a['team_id'] ?? '').toString();
    return GestureDetector(
      behavior: HitTestBehavior.opaque,
      onTap: () {
        if (teamId.isNotEmpty && team.isNotEmpty) {
          widget.onTeamTap?.call(teamId, team);
        } else {
          widget.onNavigateToTab?.call(2); // 칸반 탭
        }
      },
...
class _KpiData {
  ...
  _KpiData(this.label, this.value, this.icon, this.color, {this.sub, this.onTap});
}
