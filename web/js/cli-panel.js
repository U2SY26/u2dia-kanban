...
const CliPanel = {
  ...
  init() {
    this._el = document.getElementById('shellCliBody') || document.getElementById('wsCli');
  ...
  toggle() {
    ...
    if (!shell) { this.toggleCollapse(); return; }
    const openStates = ['', 'shell--cli-open', 'shell--cli-open-half', 'shell--cli-open-full'];
    let current = 0;
    for (let i = 1; i < openStates.length; i++) {
      if (shell.classList.contains(openStates[i])) { current = i; break; }
    }
    const next = (current + 1) % openStates.length;
    openStates.forEach(s => s && shell.classList.remove(s));
    if (openStates[next]) shell.classList.add(openStates[next]);
    const indicator = document.getElementById('shellCliIndicator');
    if (indicator) indicator.textContent = openStates[next] ? '▴ CLI' : '▾ CLI';
  },
