...
const Modal = {
  ...
  async _renderThread(body, ticketId) {
    ...
    const kindMeta = {
      conversation: { icon: 'MS', color: 'var(--cyan)',   label: '대화' },
      qa:           { icon: 'QA', color: 'var(--green)',  label: 'QA'   },
      activity:     { icon: 'PG', color: 'var(--brand)',  label: '진행' },
      artifact:     { icon: 'AR', color: 'var(--purple)', label: '산출물' },
    };
    const typeLabel = {
      meeting:   { icon: 'MT', color: 'var(--orange)', label: '회의' },
      rework:    { icon: 'RW', color: 'var(--orange)', label: '재작업' },
      response:  { icon: 'RS', color: 'var(--text)',   label: '답변' },
      qa:        { icon: 'QA', color: 'var(--green)',  label: 'QA통과' },
      fail:      { icon: 'FL', color: 'var(--danger)', label: 'QA실패' },
      question:  { icon: 'Q', color: 'var(--cyan)',   label: '질문' },
      progress:  { icon: 'PG', color: 'var(--brand)',  label: '진행' },
    };
