...
const ArchiveDetail = {
  ...
  _renderFull(container) {
    ...
    const kpiCards = [
      { v: done + '/' + tickets.length, label: '완료', color: '--chart-blue', icon: 'DN' },
      { v: progress + '%', label: '달성률', color: '--chart-green', icon: '%' },
      { v: members.length, label: '에이전트', color: '--chart-cyan', icon: 'AG' },
      { v: blocked, label: 'Blocked', color: '--chart-red', icon: 'BK' },
      { v: artifacts.length, label: '산출물', color: '--chart-orange', icon: 'AR' },
      { v: codeArts, label: '코드', color: '--brand', icon: 'CD' },
      { v: totalFiles > 0 ? '+' + linesAdded + ' -' + linesRemoved : '-', label: '변경량', color: '--chart-lime', icon: 'CHG' },
      { v: messages.length, label: '대화', color: '--chart-purple', icon: 'MSG' },
      { v: feedbacks.length, label: '피드백', color: '--yellow', icon: 'FB' },
      { v: logs.length, label: '액티비티', color: '--muted', icon: 'EV' },
    ];
  ...
  _buildEvents() {
    ...
    // 산출물
    var artTypeIcons = {code:'CD',file_path:'FP',code_change:'CH',config:'CF',test:'TS',docs:'DC',result:'RS',summary:'SM',log:'LG',diagram:'DG',screenshot:'SC',data:'DT'};
  ...
  _actIcon(action) {
    var m = {
      team_created: 'TM', ticket_created: 'TK', ticket_status_changed: 'ST',
      ticket_claimed: 'CL', member_spawned: 'AG', artifact_created: 'AR',
      feedback_created: 'FB', message_sent: 'MS', progress: 'PG',
    };
    return m[action] || 'EV';
  },
  _actColor(action) {
