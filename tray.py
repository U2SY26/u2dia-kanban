...
SETTINGS_DIR = os.path.join(os.path.expanduser("~"), ".config", "kanban-tray")
...
class ServerManager:
    def __init__(self, port: int, autostart: bool = False):
        ...
        self.notifications_enabled = bool(_saved.get("notifications_enabled", True))
    ...
    def _process_sse(self, raw: str):
        ...
        notifications = {
            ...
            "artifact_created": lambda p: (
                "산출물 등록",
                f"{p.get('title', '?')}",
            ),
            "agent_notification": lambda p: (
                {"info": "에이전트 알림", "warning": "⚠ 에이전트 경고", "critical": "🚨 긴급 알림"}.get(
                    p.get("urgency", "info"), "에이전트 알림"),
                f"[{p.get('sender_name', '?')}] {p.get('title', '')} — {p.get('body', '')}",
            ),
            "competition_updated": lambda p: (
                "대회 업데이트",
                f"{p.get('count', 0)}건 이벤트 기록",
            ),
            "lambda_costs_updated": lambda p: (
                "GPU 비용 갱신",
                f"{p.get('imported', 0)}건 동기화",
            ),
        }
        ...
        if handler:
            ...
            # critical은 반복 알림
            if event_type == "agent_notification" and event_data.get("urgency") == "critical":
    ...
    def toggle_notifications(self) -> bool:
        ...
        saved["notifications_enabled"] = self.notifications_enabled
    ...
    def _get_status_text(self) -> str:
        ...
        try:
            ...
            cost_text = ""
            try:
                resp3 = urlopen(f"{self.base_url}/api/competitions/summary", timeout=2)
                cs = json.loads(resp3.read())
                total_cost = sum(c.get("lambda_cost", 0) for c in cs.get("competitions", []))
                comp_count = len(cs.get("competitions", []))
                if total_cost > 0:
                    cost_text = f" | GPU ${total_cost:,.0f}"
                if comp_count > 0:
                    cost_text += f" | 대회 {comp_count}"
            except Exception:
                pass

            return f"실행 중 | 팀 {active}/{len(teams)} | {cost_text.lstrip(' |')}" if cost_text else f"실행 중 | 팀 {active}/{len(teams)}"
        except Exception:
            return "상태: 확인 불가"

    def _get_competitions_text(self) -> str:
        if not self.running:
            return "대회: 서버 중지"
        try:
            resp = urlopen(f"{self.base_url}/api/competitions/summary", timeout=2)
            cs = json.loads(resp.read())
            lines = []
            for c in cs.get("competitions", []):
                name = c.get("competition", "?")[:20]
                avg = c.get("avg_score")
                cost = c.get("lambda_cost", 0)
                avg_str = f"{avg:.1f}" if avg else "-"
                lines.append(f"{name}: avg {avg_str}, ${cost:,.0f}")
            return " | ".join(lines) if lines else "대회 없음"
        except Exception:
            return "대회: 조회 실패"

    def open_competitions(self):
        webbrowser.open(f"{self.base_url}/#/competitions")
    ...
    def run(self):
        ...
        def on_competitions(icon, item):
            self.open_competitions()
        ...
        menu = pystray.Menu(
            ...
            pystray.MenuItem(
                lambda item: self._get_competitions_text(),
                None,
                enabled=False,
            ),
            ...
            pystray.MenuItem("대회 현황", on_competitions),
